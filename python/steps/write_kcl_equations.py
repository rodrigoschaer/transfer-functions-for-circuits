        ...
        for name, comp_nodes, value in connections:
            if name.startswith("R"):
                ...
                if comp_nodes[0] == node:
                    node_current += current_through_resistor(
                        node_voltages[node], node_voltages[comp_nodes[1]], symbols[name]
                    )
                else:
                    node_current += current_through_resistor(
                        node_voltages[comp_nodes[1]], node_voltages[node], symbols[name]
                    )
            elif name.startswith("C"):
                # Capacitor
                if comp_nodes[0] == node:
                    node_current += current_through_capacitor(
                        node_voltages[node],
                ...
                # Inductor
                if comp_nodes[0] == node:
                    node_current += current_through_inductor(
                        node_voltages[node],
                        node_voltages[comp_nodes[1]],
                        symbols[name],
                        s,
